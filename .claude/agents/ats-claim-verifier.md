...
- **Note:** *(optional)* nuance, related file, caveat

Read-only for source (no Edit/Write); Bash is inspection-only (grep, find,
git log/blame, counting) — never mutating.
...
- **`nx test api`'s `testMatch` only globs `src/app/**/*.spec.ts`.** Specs
  ...
  `jest --config apps/api/jest.config.ts --testMatch "**/src/common/**/*.spec.ts"`
  invocation (NOT `--testPathPatterns`, which filters on top of `testMatch`
  and won't surface anything under `src/common` since `testMatch` already
...
- **Nx auto-injects `.env` into every task it runs, and overrides an
  *empty* shell-exported variable with the `.env` value** (but respects a
  *non-empty* shell override). So `VAR= nx serve api` does NOT actually
