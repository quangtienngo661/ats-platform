...
export function CandidateOverviewTab({
  ...
  if (screening?.matchedSkills) {
    ...
    if (typeof parsedMatch === 'string') {
      try {
        parsedMatch = JSON.parse(parsedMatch);
      } catch {
        /* ignore */
      }
    }
  ...
  if (screening?.missingSkills) {
    ...
    if (typeof parsedMiss === 'string') {
      try {
        parsedMiss = JSON.parse(parsedMiss);
      } catch {
        /* ignore */
      }
    }
    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
      ...
      <div className="lg:col-span-2 space-y-5">
        ...
        <div className="bg-white rounded-2xl border border-[#F2F2F7] p-6">
          <h2
            className="text-[16px] text-[#1D1D1F] mb-4"
          ...
          <div className="grid grid-cols-2 gap-4">
            <InfoItem
              icon={Briefcase}
              label="Vị trí"
              value={application.jobPosting?.title || 'N/A'}
            />
            <InfoItem
              icon={MapPin}
              label="Hình thức"
              value={
                LOCATION_LABELS[
                  application.jobPosting?.locationType || 'N/A'
                ] || 'N/A'
              }
            />
            <InfoItem
            ...
            <InfoItem
              icon={Clock}
              label="Trạng thái sàng lọc"
              value={screeningStatus === 'completed' ? 'Hoàn tất' : 'Đang chờ'}
            />
          </div>
        </div>
        ...
        {screeningStatus === 'completed' ? (
            ...
            <div className="bg-white rounded-2xl border border-[#F2F2F7] p-6">
              ...
              <div className="p-4 rounded-xl bg-[#F5F5F7] mb-4">
                ...
                >
                  ...
                  .
                  {recommendation === 'interview' &&
                    ' Nên sắp xếp phỏng vấn để đánh giá chi tiết hơn.'}
                  {recommendation === 'reject' &&
                    ' Hồ sơ chưa đáp ứng đủ yêu cầu cho vị trí này.'}
                </p>
              </div>
              {aiReasoning && (
                <div className="p-4 rounded-xl border border-[#E5E5EA] bg-white">
                  <h3
                    className="text-[13px] text-[#6E6E73] mb-2"
                    style={{ fontWeight: 600, textTransform: 'uppercase' }}
                  >
                    Lý do từ AI:
                  </h3>
                  <p
                    className="text-[14px] text-[#1D1D1F] leading-relaxed whitespace-pre-wrap"
                    style={{ fontFamily: SFT }}
                  >
                    {aiReasoning}
                  </p>
          ...
          <div className="bg-white rounded-2xl border border-[#F2F2F7] p-10 text-center">
            <p
              className="text-[16px] text-[#6E6E73]"
              style={{ fontWeight: 500 }}
            >
              {screeningStatus === 'processing'
                ? 'Đang sàng lọc CV...'
                : 'Chưa bắt đầu sàng lọc'}
            </p>
            <p className="text-[13px] text-[#AEAEB2] mt-2">
              Kết quả đánh giá AI sẽ hiển thị khi quá trình sàng lọc hoàn tất.
            </p>
          </div>
        )}
      </div>
      ...
      <div className="space-y-5">
        ...
        {screeningStatus === 'completed' && (
          <div className="bg-white rounded-2xl border border-[#F2F2F7] p-6 text-center">
            <h2
              className="text-[15px] text-[#1D1D1F] mb-4"
              style={{ fontFamily: SF, fontWeight: 600 }}
            >
              Điểm tổng AI
            </h2>
            <div className="relative inline-flex items-center justify-center w-36 h-36">
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center">
                <span
                  className="text-[32px] text-[#1D1D1F]"
            ...
            <p
              ...
              style={{ fontWeight: 600, color: getScoreColor(overallScore) }}
            >
              {recLabel}
            </p>
          </div>
          ...
          (matchedHardSkills.length > 0 ||
            ...
            <div className="bg-white rounded-2xl border border-[#F2F2F7] p-6">
              ...
              {matchedHardSkills.length > 0 && (
                <div className="mb-4">
                  <p
                    className="text-[12px] text-[#34C759] mb-2"
                    style={{ fontWeight: 600, textTransform: 'uppercase' }}
                  >
                    Đáp ứng (Bắt buộc)
                  </p>
                  <div className="flex flex-wrap gap-1.5">
                    {matchedHardSkills.map((skill, idx) => (
                      <span
                        key={idx}
                        className="text-[12px] bg-[#F0FDF4] border border-[#34C759]/20 text-[#34C759] px-2.5 py-1 rounded-full"
                      >
                        {skill}
                      </span>
                    ))}
              ...
              {matchedNiceToHaveSkills.length > 0 && (
                <div className="mb-4">
                  <p
                    className="text-[12px] text-[#0071E3] mb-2"
                    style={{ fontWeight: 600, textTransform: 'uppercase' }}
                  >
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {matchedNiceToHaveSkills.map((skill, idx) => (
                      <span
                        key={idx}
                        className="text-[12px] bg-[#EBF3FD] border border-[#0071E3]/20 text-[#0071E3] px-2.5 py-1 rounded-full"
                      >
                        {skill}
                      </span>
                    ))}
              ...
              {missingSkillsList.length > 0 && (
                <>
                  ...
                  <div>
                    <p
                      className="text-[12px] text-[#EF4444] mb-2"
                      style={{ fontWeight: 600, textTransform: 'uppercase' }}
                    >
                      Thiếu sót
                    </p>
                    <div className="flex flex-wrap gap-1.5">
                      {missingSkillsList.map((skill, idx) => (
                        <span
                          key={idx}
                          className="text-[12px] bg-[#FEF2F2] border border-[#EF4444]/20 text-[#EF4444] px-2.5 py-1 rounded-full"
                        >
                          {skill}
                        </span>
                      ))}
...
function InfoItem({
  ...
  return (
    <div className="flex items-start gap-3">
      ...
      <div>
        <p
          className="text-[11px] text-[#AEAEB2] uppercase tracking-wide"
          style={{ fontWeight: 500 }}
        >
          {label}
        </p>
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-1.5">
        ...
        <span
          className="text-[13px]"
          style={{ fontWeight: 600, color: getScoreColor(score) }}
        >
          {score}
        </span>
      </div>
