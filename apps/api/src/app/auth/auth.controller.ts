...
export class AuthController {
  ...
  @Post('refresh')
  @ApiOperation({ summary: 'Làm mới token', description: 'Sử dụng refreshToken trong cookie để lấy accessToken mới.' })
  @ApiResponse({ status: 200, description: 'Làm mới thành công' })
  @ApiResponse({ status: 401, description: 'RefreshToken không hợp lệ hoặc đã hết hạn' })
  ...
  @Post('logout')
  @ApiOperation({ summary: 'Đăng xuất', description: 'Thu hồi refreshToken và xóa cookie phiên đăng nhập.' })
  ...
  @ApiOperation({ summary: 'Gửi email xác thực', description: 'Gửi lại email xác thực hoặc email đặt lại mật khẩu.' })
  ...
  @Get('verify-email')
  @ApiOperation({ summary: 'Xác thực email', description: 'Xác thực email qua token gửi trong URL. Redirect về client sau khi xác thực.' })
  @ApiQuery({ name: 'token', required: false, description: 'Token xác thực' })
  @ApiQuery({ name: 'type', required: false, description: 'Loại xác thực (verify | reset)' })
  @ApiResponse({ status: 302, description: 'Redirect về trang kết quả xác thực' })
  async verifyEmail(
    @Res() res: Response,
  ...
  @Post('reset-password')
  @ApiOperation({ summary: 'Đặt lại mật khẩu', description: 'Đặt lại mật khẩu bằng token từ email.' })
