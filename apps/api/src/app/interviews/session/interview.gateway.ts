...
export class InterviewGateway {
  ...
  async handleJoinSession(
    ...
    try {
      ...
      // Đánh dấu phiên vẫn còn sống, để job quét không hủy nhầm.
      await this.interviewSessionService.touchActivity(sessionId);
      ...
      const status =
        await this.interviewSessionService.getSessionStatus(sessionId);

      if (status === InterviewStatus.generating) {
        client.emit('interview:session_generating', {
          message:
            'AI đang tạo câu hỏi phỏng vấn, vui lòng chờ trong giây lát...',
        });
        return;
      }
      ...
      const currentQuestion =
        await this.interviewSessionService.getCurrentQuestion(sessionId);
      ...
      client.emit('interview:error', {
        message: this.toClientMessage(
          error,
          'Không thể tham gia phiên phỏng vấn. Vui lòng thử lại.',
        ),
      });
    }
  }
  ...
  async handleSubmitAnswer(
    @ConnectedSocket() client: Socket,
    @MessageBody()
    data: { sessionId: string; qnaId: string; answerText: string },
  ) {
    try {
      ...
      switch (result.type) {
        ...
        case 'end': {
          ...
          const finalResult =
            await this.interviewSessionService.endSession(sessionId);
  ...
  async handleSubmitFollowup(
    @ConnectedSocket() client: Socket,
    @MessageBody()
    data: { sessionId: string; qnaId: string; followupAnswer: string },
  ) {
    try {
      ...
      client.emit('interview:error', {
        message: this.toClientMessage(
          error,
          'Không thể gửi câu trả lời phụ. Vui lòng thử lại.',
        ),
      });
    }
  }
  ...
  private async verifySessionOwnership(
    ...
    const isOwner = await this.interviewSessionService.verifySessionOwnership(
      userId,
      sessionId,
    );
    ...
    if (!isOwner) {
      client.emit('interview:error', {
        message: 'Bạn không có quyền truy cập phiên phỏng vấn này',
      });
      return false;
    }
