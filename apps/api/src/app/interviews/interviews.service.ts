...
export class InterviewsService {
    ...
    private async assertCanScheduleApplication(userId: string, role: string, applicationId: string) {
        ...
        if (application.status !== ApplicationStatus.interview) {
            throw new BadRequestException('Chỉ có thể đặt lịch phỏng vấn cho đơn đang ở trạng thái phỏng vấn');
        }

        if (role === UserRole.admin) return application;
        if (role !== UserRole.recruiter) {
            throw new ForbiddenException('Bạn không có quyền lên lịch phỏng vấn cho đơn ứng tuyển này');
        }
    ...
    private async assertCanUseInterviewer(userId: string, role: string, interviewerId: string, departmentId: string) {
        ...
        if (!interviewer) {
            throw new BadRequestException('Người phỏng vấn phải là tài khoản nhà tuyển dụng');
        }
        ...
        const recruiterDepartmentId = await this.getRecruiterDepartmentId(userId);
        if (recruiterDepartmentId !== departmentId || interviewer.departmentId !== departmentId) {
            throw new ForbiddenException('Người phỏng vấn phải thuộc cùng khoa với đơn ứng tuyển');
        }
    }
    ...
    private async assertCanViewSchedule(userId: string, role: string, interviewId: string) {
        ...
        if (role === UserRole.admin) return;
        if (role === UserRole.candidate && schedule.application.candidate.userId === userId) return;
        if (role === UserRole.recruiter) {
            if (schedule.scheduledBy === userId || schedule.interviewerId === userId) return;

            const recruiterDepartmentId = await this.getRecruiterDepartmentId(userId);
            if (recruiterDepartmentId === schedule.application.jobPosting.departmentId) return;
        }

        throw new ForbiddenException('Bạn không có quyền truy cập lịch phỏng vấn này');
    }

    private async assertCanMutateSchedule(userId: string, role: string, interviewId: string) {
        const schedule = await this.prisma.interviewSchedule.findUnique({
            where: { interviewId },
            select: {
                interviewId: true,
        ...
        if (!schedule) throw new NotFoundException('Không tìm thấy lịch phỏng vấn');
        if (role === UserRole.admin || schedule.scheduledBy === userId) return schedule;
    ...
    async getMySchedules(userId: string, role: string, query: GetInterviewSchedulesQueryDto) {
        ...
        if (query.fromDate || query.toDate) {
            ...
            if (query.toDate) where.scheduledDate.lte = this.getEndOfDay(query.toDate);
        }
    ...
    async updateSchedule(interviewId: string, userId: string, role: string, dto: UpdateInterviewScheduleDto) {
        const schedule = await this.assertCanMutateSchedule(userId, role, interviewId);
    ...
    async updateTopic(topicId: string, dto: UpdateInterviewTopicDto) {
        ...
        if (dto.categoryId !== undefined) {
            const categoryId = dto.categoryId?.trim() || null;
            await this.assertTopicCategoryExists(categoryId);
            updateData.category = categoryId ? { connect: { categoryId } } : { disconnect: true };
        }
    ...
    async removeTopic(topicId: string) {
        ...
        if (topic._count.sessions > 0) {
            throw new BadRequestException('Không thể xóa chủ đề đã có phiên phỏng vấn');
        }
    ...
    async getSessionResult(sessionId: string, userId: string) {
        ...
        if (!result) throw new NotFoundException('Không tìm thấy kết quả phỏng vấn');
