...
/**
 * How many times a recruiter may re-trigger a screening that keeps failing.
 * `CVScreening.retryCount` is incremented on every manual re-trigger; without a
 * cap a permanently-broken CV could be re-queued (and re-billed to Gemini) forever.
 */
...
const APPLICATION_STATUS_NOTIFICATION_COPY: Record<
  string,
...
> = {
  ...
  [ApplicationStatus.offer]: {
    ...
    message:
      'Đơn ứng tuyển của bạn đã được chuyển sang vòng đề nghị tuyển dụng.',
  },
  [ApplicationStatus.hired]: {
    title: 'Chúc mừng, bạn đã được tuyển',
    message:
      'Chúc mừng, đơn ứng tuyển của bạn đã được chuyển sang trạng thái trúng tuyển.',
  },
  [ApplicationStatus.rejected]: {
...
export class ApplicationsService {
  ...
  /**
   * Every status this application has actually been in, oldest first. Derived from
   * `ApplicationHistory.fromStatus`, plus `applied` — every application starts there
   * and the first history row records leaving it, so it is never a `fromStatus`
   * unless it was left. This is the only set a revert is allowed to target.
   */
  private async getPreviouslyHeldStatuses(
    ...
    const held = new Set<ApplicationStatus>([ApplicationStatus.applied]);
    for (const row of history) {
      if (row.fromStatus) held.add(row.fromStatus);
    }
  ...
  async apply(userId: string, dto: CreateApplicationDto) {
    ...
    if (job.status !== JobStatus.active) {
      throw new BadRequestException(
        'Tin tuyển dụng này hiện không nhận đơn ứng tuyển',
      );
    }
    ...
    if (cv.parsingStatus !== ParsingStatus.completed) {
      throw new BadRequestException(
        'CV của bạn cần được phân tích thành công trước khi ứng tuyển',
      );
    }
    if (!cv.parsedData?.isConfirmed) {
      throw new BadRequestException(
        'Bạn cần xác nhận hồ sơ CV trước khi ứng tuyển',
      );
    }
    ...
    const isNotCancelled = existings.some(
  ...
  async getAllKanbanBoard(userId: string, role: string) {
    ...
    const board = activeStatuses.reduce(
      (acc, status) => {
        acc[status] = [];
        return acc;
      },
  ...
  async updateStatus(
    ...
    if (dto.isReverted) {
      // A revert may only go BACK to a status this application actually held before.
      // Without this, `isReverted: true` was a skeleton key: it skipped
      // VALID_TRANSITIONS entirely, so `applied` → `hired` in one call was accepted.
      const previouslyHeld =
        await this.getPreviouslyHeldStatuses(applicationId);

      if (dto.status === application.status) {
        throw new BadRequestException(
          `Đơn ứng tuyển đã ở trạng thái '${dto.status}'`,
        );
      }

      if (!previouslyHeld.includes(dto.status)) {
        throw new BadRequestException(
          `Không thể hoàn tác về trạng thái '${dto.status}' vì đơn ứng tuyển chưa từng ở trạng thái này. ` +
            `Các trạng thái có thể hoàn tác: [${previouslyHeld.join(', ') || 'không có'}]`,
        );
      }
    } else {
  ...
  async triggerScreening(
    ...
    const application = await this.prisma.application.findUnique({
      ...
      select: {
        ...
        cvId: true,
        screening: {
          select: { screeningId: true, status: true, retryCount: true },
        },
      },
    });
    if (!application)
    ...
    if (application.screening && application.screening.status) {
      ...
      if (application.screening.status === ScreeningStatus.completed) {
        throw new ConflictException(
          'Sàng lọc AI đã hoàn tất. Vui lòng xem kết quả trong chi tiết đơn ứng tuyển',
        );
      }
      if (
        application.screening.status === ScreeningStatus.failed &&
        application.screening.retryCount >= MAX_SCREENING_RETRIES
      ) {
        throw new ConflictException(
          `Sàng lọc AI đã thất bại ${MAX_SCREENING_RETRIES} lần. Vui lòng kiểm tra lại CV hoặc liên hệ quản trị viên`,
        );
      }
    }
